...
//    val upsertData = UpsertData.create(idId, email, name, name, None, Some("n19gu"),uk, None, allowMail, allowMail, allowMail)
//
...
//    val service = new SalesforceService(Configuration.salesforceConfigProvider.get(), configurableFutureRunner(10.seconds))
//    val upsertData = UpsertData.create(idId, email, name, name, None, Some("n19gu"),uk, Some(telephoneNumber), allowMail, allowMail, allowMail)
//
//    service.upsert(upsertData).map { response: SalesforceContactResponse =>
//      response.Success should be(true)
//      response.ContactRecord.Id should be(salesforceId)
